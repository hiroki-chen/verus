...
impl<'a> Elf64File<'a> {
    ...
    pub fn read(elf_file_buf: &'a [u8]) -> Result<Self, ElfError> {
        ...
        // Second pass: verify sections now that we have the string table
        for i in 0..elf_hdr.e_shnum {
            let shdr = Self::read_shdr_from_file(elf_file_buf, &elf_hdr, i);
            Self::verify_shdr(&shdr, elf_file_buf.len(), elf_hdr.e_shnum, sh_strtab.as_ref())?;
        }
    ...
    /// the header is valid. Debug sections and other unneeded sections are skipped.
    ///
    /// # Arguments
    ///
    ...
    /// * `sh_strtab` - Optional section header string table for getting section names.
    ///
    /// # Errors
    ///
    ...
    fn verify_shdr(
        ...
        elf_file_buf_len: usize,
        shnum: Elf64Word,
        sh_strtab: Option<&Elf64Strtab>,
    ) -> Result<(), ElfError> {
        if shdr.sh_type == Elf64Shdr::SHT_NULL {
            return Ok(());
        }

        // Skip validation for debug sections and other unneeded sections
        if let Some(strtab) = sh_strtab {
            if let Ok(section_name_cstr) = strtab.get_str(shdr.sh_name) {
                if let Ok(section_name) = section_name_cstr.to_str() {
                    // Skip debug sections, comment sections, and symbol tables
