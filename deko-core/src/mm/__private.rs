...
//!
//! Fix: support for 2MB paging.
use deko_std::prelude::*;
use vstd::prelude::*;

use crate::mm::{
    phys_to_virt, virt_to_phys, DEKO_FRAME_ALLOCATOR, PTE_MASK_PRIVATE, PTE_MASK_SHARED,
};
...
extern "C" {
    ...
    #[link_name = "pgtable"]
    pub static mut pgtable___: [u64; 512];
}
...
impl PageTable {
...

    {
        ...
        Self::walk_level3(root, vaddr, root_perm, debug)
    }
            ...
            r.wf(),
    {
        ...
        Self::walk_level1(lv2_page, vaddr, lv2_page_perm, debug)
    }
            ...
            r.wf(),
    {
        ...
        match PageTableEntry::flags_from_ptr(entry, Tracked(&entry_perm)).contains(PRESENT) {
            ...
            true => {
                ...
                Self::walk_level2(lv3_page, vaddr, lv3_page_perm, debug)
            },
        }
    }
...

    {
        ...
        PageTableEntry::set(pte, Tracked(&mut perm), make_private_address(paddr).into(), flags);
    ...
    // vaddr.wf(),
        ...
        ensures
            ...
            r.lvl() == 0,
    {
        let m = Self::walk(pt, vaddr, Tracked(pt_perm), false);
    ...
    // vaddr.wf(),
            ...
            pt_perm.is_init(),
    {
        // 09/08: this is problematic again.
        let mapping = Self::walk(pt, vaddr, Tracked(pt_perm), true);  // <- ?? exceptions
        if let Mapping::Level0(entry, perm) = mapping {
            let Tracked(mut perm) = perm;

            PageTableEntry::set_pte_shared(entry, Tracked(&mut perm));
        } else {
            vstd::vpanic!("page not mapped");
        }
    }
